		/*----------------------------------------------------------------------------
		* Name:    mbprocess.c
		* Purpose: MODBUS Process usage for STM32
		* Version: V1.00
		*----------------------------------------------------------------------------*/
...
void MBSendData(unsigned char count)	//Send final data over UART
{
	...
	{
		while( !( USART2->SR & USART2_TXE )) {}	//wait till transmit buffer is empty
		USART2->DR = data_in[c];
	}
}
...
unsigned int MBRegisterCount(void)
{
	return (uint16_t)(data_in[5]|(data_in[4]<<8));
}

unsigned int MBStartAddress(void)	//Return requested start address
{
	return (uint16_t) (data_in[3]|(data_in[2]<<8));
}
